...
RSpec.describe "Petals::Viewport::DEFAULT_KEY_MAP" do
  let(:key_map) { Petals::Viewport::DEFAULT_KEY_MAP }
  ...
  it "defines half_page_up action" do
    expect(key_map).to have_key(:half_page_up)
  end

  it "defines half_page_down action" do
    expect(key_map).to have_key(:half_page_down)
  end
  ...
  it "maps space to page_down" do
    msg = Chamomile::KeyMsg.new(key: " ", mod: [])
    expect(Petals::KeyBinding.key_matches?(msg, key_map, :page_down)).to be true
  end
end
